...
/* ══════════════════════════════════════════════════════
   Slider A/B — Pool: side A = day video (images/3.webm), side B = night
   video (images/3_night.webm). Both videos always play in parallel behind
   the canvas — switching sides is just an opacity toggle (no src/load/
   fade), and the middle of the drag scrubs through a pre-rendered frame
   sequence (same mechanism as the bidese.html dn-slider).
   ══════════════════════════════════════════════════════ */

const AB_SEQUENCE_ID = "pool-ab-reveal";
const abVideoB = document.getElementById("ab-video-b");
const stageEl = document.getElementById("stage");
let abFrames = null, abLoading = null, abDragging = false, abT = 0;
...
function setAbSliderVisible(visible) {
  ...
  if (visible) {
    abEnsureFrames();
    const src = CONFIG.scenes.pool && CONFIG.scenes.pool.abVideoB;
    if (src) {
      if (!abVideoB.src) {
        abVideoB.src = videoBlobs.get(src) || src;
        abVideoB.load();
      }
      abVideoB.play().catch(() => {});
    }
...
/* ══════════════════════════════════════════════════════
   ...
   transition never hits a missing frame.
   ══════════════════════════════════════════════════════ */
